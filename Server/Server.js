...
app.post('/get-ticket-link', async (req, res) => {
  try {
    const { paymentMethod, amount, phoneNumber, password, ticketId, userId } = req.body;
...
app.delete('/reservation/:id', async (req, res) => {
  try {
    ...
    const deletedPayment = await Payment.findByIdAndDelete(paymentId, { password: 0 });
...
app.post("/book-ticket", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ ticket: ticket, message: "Ticket booked successfully",ticketId:ticket._id.toString() });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Internal server error" });
  }
...
app.post('/save-missing-person-report', upload.array('photos', 5), async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Missing person report saved successfully' });
  } catch (error) {
